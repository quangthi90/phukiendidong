...
class ControllerCatalogAttribute extends Controller { 
	...
	public function loadattributebygroup() {
		$json = array();
		if (isset($this->request->get['attribute_group_id'])) {
			$this->load->model('catalog/attribute');
			$json['data'] = $this->model_catalog_attribute->getAttributesByAttributeGroupId(array('filter_attribute_group_id' => $this->request->get['attribute_group_id']));
			if ($json['data']) {
				$json['status'] = 'success';
			}else {
				$json['status'] = 'failed';
