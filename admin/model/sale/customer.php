...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
      	$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', sex = '" . $this->db->escape($data['sex']) . "', birthday = '" . strtotime($data['birthday']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', password = '" . $this->db->escape(md5($data['password'])) . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
      	...
      	if (isset($data['address'])) {		
      		foreach ($data['address'] as $address) {	
      			$this->db->query("
      				...
      				SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', 
      					lastname = '" . $this->db->escape($address['lastname']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "'");
				...
				if (isset($address['default'])) {
					...
					$this->db->query("
						...
						SET address_id = '" . $address_id . "' WHERE customer_id = '" . (int)$customer_id . "'");
				}
			}
		}
	}
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("
			...
			SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', 
				email = '" . $this->db->escape($data['email']) . "', sex = '" . $this->db->escape($data['sex']) . "', 
				birthday = '" . strtotime($data['birthday']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', 
      	...
      	if (isset($data['address'])) {
      		foreach ($data['address'] as $address) {
				$this->db->query("
					INSERT INTO " . DB_PREFIX . "address 
					SET address_id = '" . (int)$address['address_id'] . "', customer_id = '" . (int)$customer_id . "', 
						firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', 
...
//		print $sql; exit;
	...
	public function approve($customer_id) {
		...
		if ($customer_info) {
			...
			$login_url = 'http://www.inhere.vn/dang-nhap';
			
			$message  = sprintf($this->language->get('text_approve_welcome'), $customer_info['firstname'] . ' ' . $customer_info['lastname']) . "\n\n";
			...
			$mail->setSubject(html_entity_decode(sprintf($this->language->get('text_approve_subject'), $customer_info['firstname'] . ' ' . $customer_info['lastname']), ENT_QUOTES, 'UTF-8'));
