...
class ModelLocalisationCurrency extends Model {
	...
	public function updateCurrencies($force = false) {
		if (extension_loaded('curl')) {
			...
			if ($this->config->get('config_currency') == 'VND') {
				foreach ($query->rows as $result) {
					$data[] = $result['code'];
				}
    			...
    			foreach($xmlData as $v)
        			if(isset($v['attributes'])) {
            			if (in_array($v['attributes']['CURRENCYCODE'], $data)) {
                			$this->db->query("UPDATE " . DB_PREFIX . "currency SET value = '" . (1/$v['attributes']['BUY']) . "', date_modified = '" .  $this->db->escape(date('Y-m-d H:i:s')) . "' WHERE code = '" . $this->db->escape($v['attributes']['CURRENCYCODE']) . "'");
            			}
            		}
			}else {
				foreach ($query->rows as $result) {
