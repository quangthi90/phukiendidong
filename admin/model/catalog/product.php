...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		$this->db->query(
			...
			SET model = '" . $this->db->escape($data['model']) . "', quantity = '" . (int)$data['quantity'] . "', 
				...
				status = '" . (int)$data['status'] . "', label = '" . (int)$data['product_label'] . "', 
		...
		foreach ($data['product_description'] as $language_id => $value) {
			...
			if (isset($value['meta_description'])) {
				foreach ($value['meta_description'] as $item) {
					if ($item) {
			...
			if ($meta_description) {
				$meta_description = $this->db->escape(serialize($meta_description));
			}else {
				$meta_description = '';
			}
			...
			$this->db->query("
				...
				SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', 
					name = '" . $this->db->escape($value['name']) . "', meta_description = '" . $meta_description . "', 
		...
		*/
		...
		if (isset($data['keyword']) && !empty($data['keyword'])) { 
			...
			$remove_sign = $this->url->removeSign($data['product_description'][2]['name']);
			
			$keyword = $remove_sign . '-' . $product_id;
	...
	public function getProductLabel($product_id) {
		$sql = "SELECT label FROM " . DB_PREFIX . "product WHERE product_id='" . $product_id . "'";
